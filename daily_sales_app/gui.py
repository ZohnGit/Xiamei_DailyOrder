...
class DailySalesApp(tk.Tk):
    ...
    def _build_main_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(upload_frame, text="上一步", command=self.go_previous_platform).grid(
            row=1, column=2, sticky="ew", padx=(0, 12), pady=(0, 12)
        )
        ...
        self.progress_tree.column("operator", width=120, anchor="center")
    ...
    def _build_config_tab(self, parent: ttk.Frame) -> None:
        ...
        self.config_tree.column("operator", width=160, anchor="center")
        ...
        ttk.Label(right, text="平台名称").grid(row=0, column=0, sticky="w", padx=12, pady=(12, 4))
        ttk.Entry(right, textvariable=self.platform_name_var, width=24).grid(row=1, column=0, sticky="ew", padx=12)

        ttk.Label(right, text="运营名称").grid(row=2, column=0, sticky="w", padx=12, pady=(12, 4))
        ttk.Entry(right, textvariable=self.operator_name_var, width=24).grid(row=3, column=0, sticky="ew", padx=12)

        ttk.Checkbutton(right, text="需要成本报表", variable=self.requires_cost_var).grid(
            row=4, column=0, sticky="w", padx=12, pady=(12, 0)
        )

        ttk.Button(right, text="新增到末尾", command=self.add_platform).grid(row=5, column=0, sticky="ew", padx=12, pady=(18, 6))
        ttk.Button(right, text="更新当前项", command=self.update_selected_platform).grid(row=6, column=0, sticky="ew", padx=12, pady=6)
        ttk.Button(right, text="删除当前项", command=self.delete_selected_platform).grid(row=7, column=0, sticky="ew", padx=12, pady=6)
        ttk.Button(right, text="保存配置", command=self.save_platform_config).grid(row=8, column=0, sticky="ew", padx=12, pady=(18, 6))

        note = (
            "说明:\n"
            "1. 新增平台默认追加到上传顺序末尾。\n"
            "2. 需要成本报表的平台，生成前会强制校验。\n"
            "3. 保存后会立即刷新每日上传顺序。"
        )
        ttk.Label(right, text=note, justify="left").grid(row=9, column=0, sticky="w", padx=12, pady=(18, 12))
    ...
    def _refresh_prompt(self) -> None:
        ...
        self.current_platform_path.set(self.platform_files.get(current_name, ""))
    ...
    def _validate_before_generate(self) -> tuple[bool, str]:
        ...
        for config in self.platform_configs:
            ...
            if not self.platform_files.get(config["name"]):
    ...
    def generate_report(self) -> None:
        ...
        try:
            ...
            result = process_daily_sales(
                ...
                platform_files={name: Path(path) for name, path in self.platform_files.items()},
