...
class DailySalesApp(tk.Tk):
    ...
    def _build_main_tab(self, parent: ttk.Frame) -> None:
        ...
        progress_header.pack(anchor="w", fill="x")
        ttk.Label(progress_header, text="上传进度").pack(side="left")
        ttk.Button(progress_header, text="↻", width=3, command=self.refresh_progress_view).pack(side="left", padx=(6, 0))
    ...
    def refresh_progress_view(self) -> None:
        self._refresh_platforms(reset_index=False)
        self.status_text.set("上传进度已刷新。")
