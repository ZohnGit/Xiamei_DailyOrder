...
COST_TOTAL_ALIASES = ["总成本", "总成本（人民币）", "总成本(人民币)", "总成本人民币"]
COST_EUR_TOTAL_ALIASES = ["总成本（欧元）", "总成本(欧元)", "总成本欧元"]
...
def build_cost_index(cost_report_path: Path, eur_to_cny_rate: float) -> list[dict[str, Any]]:
    ...
    eur_total_key = find_column_by_alias(columns, COST_EUR_TOTAL_ALIASES)
    ...
    if not total_key and not eur_total_key:
    ...
    for row in cost_rows:
        ...
        qty = parse_number(row.get(qty_key)) if qty_key else 0.0

        eur_total_cost = parse_number(row.get(eur_total_key)) if eur_total_key else 0.0
        if not eur_total_cost and total_key:
            cny_total_cost = parse_number(row.get(total_key))
            if cny_total_cost and eur_to_cny_rate:
                eur_total_cost = round(cny_total_cost / eur_to_cny_rate, 2)

        unit_cost = parse_number(row.get(unit_key)) if unit_key else 0.0
        if not unit_cost and eur_total_cost and qty:
            unit_cost = round(eur_total_cost / qty, 2)

        normalized_rows.append(
            {
                "order_no": order_no,
                "sku_detail": sku_detail,
                "eur_total_cost": eur_total_cost,
                "unit_cost": unit_cost,
            }
        )
...
def match_cost(cost_rows: list[dict[str, Any]], reference_no: Any, sku: Any, qty: float) -> float | None:
    ...
    total_cost = matched["eur_total_cost"]
    if unit_cost:
